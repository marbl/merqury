...
echo -e "\t<input.fofn>: fastq.gz files we want to split by every 40 million lines."
...
if [[ ${fq##*.} == "gz" ]]; then
	echo "\
	zcat $fq | split -a 4 -d -l $LINES_PER_FILE --additional-suffix=.fq   - $tmp/$fq_prefix."
	zcat $fq | split -a 4 -d -l $LINES_PER_FILE --additional-suffix=".fq" - $tmp/$fq_prefix.
else
	echo "\
	split -a 4 -d -l $LINES_PER_FILE --additional-suffix=.fq   $fq $tmp/$fq_prefix."
	split -a 4 -d -l $LINES_PER_FILE --additional-suffix=".fq" $fq $tmp/$fq_prefix.
fi

echo "
pigz --processes $cpus $tmp/$fq_prefix.*.fq"
pigz --processes $cpus $tmp/$fq_prefix.*.fq

echo "
mv $tmp/$fq_prefix.*.fq.gz split/"
mv $tmp/$fq_prefix.*.fq.gz split/
